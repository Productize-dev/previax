...
export function NetflixVideoOverlay({
  ...
  /** Keep chrome up while the pointer is over our controls. */
  ...
  // Reset chrome when switching models
  useEffect(() => {
    if (!open) return;
    revealChrome();
  }, [homeIndex, open, revealChrome]);
  ...
  return (
    ...
    >
      <div className="relative size-full">
        ...
        {/*
          YouTube iframes swallow mousemove. When chrome is hidden, this layer
          sits above the iframe so any mouse movement can bring controls back.
        */}
        {!chromeVisible && (
          <div
        ...
        <div
          className={cn(
            "pointer-events-none absolute inset-x-0 top-0 z-20 bg-gradient-to-b from-black/80 via-black/40 to-transparent px-[4%] pt-3 pb-16 transition-opacity duration-300",
        ...
        {/* Bottom chrome — prev / next */}
        <div
          className={cn(
            "pointer-events-none absolute inset-x-0 bottom-0 z-20 bg-gradient-to-t from-black/85 via-black/45 to-transparent px-[4%] pt-16 pb-5 transition-opacity duration-300",
            chromeVisible ? "opacity-100" : "opacity-0",
          )}
        >
          <div
